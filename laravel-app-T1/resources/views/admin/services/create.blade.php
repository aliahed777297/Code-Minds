        ...
        <form action="{{ route('admin.services.store') }}" method="POST">
            ...
            <div style="display:flex; gap:8px; margin-bottom:12px">
                <div style="flex:1">
                    ...
                    <input type="number" name="price" step="0.01" value="{{ old('price') }}" required style="width:100%;padding:8px">
                    @error('price') <div style="color:red">{{ $message }}</div> @enderror
                </div>
                <div style="width:140px">
                    <label>المدة (دقائق)</label>
                    <input type="number" name="duration_minutes" value="{{ old('duration_minutes') }}" style="width:100%;padding:8px">
